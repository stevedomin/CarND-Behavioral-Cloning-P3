...
def load_dataset(dataset):
    ...
    # here we add a column to the Dataframe to identity the dataset
    df[7] = dataset
    return df

def load_datasets(datasets):
    dfs = (load_dataset(dataset) for dataset in datasets)
    # merge all the datasets into one big Dataframe
    df = pd.concat(dfs, axis=0, ignore_index=True)
    return df.values

def load_image(dataset, source_path):
    filename = source_path.split('/')[-1]
    current_path = join(dataset_root_path, dataset, 'IMG', filenamev)
    return cv2.imread(current_path)

def preprocess_sample(sample, images, measurements):
    # steering measurement correction for the left and right camera image
    ...
    # The datasets are heavily skewed towards neutral steering angle so we
    # drop some of these images
    if abs(steering_center) < 0.05 and np.random.random_sample() <= 0.26:
        return

    # For each image (center, left, right) in the sample we generate a
...
model = Sequential()
# remove top and bottom part of the image
model.add(Cropping2D(cropping=((60, 25), (0,0)), input_shape=(160,320,3)))
# normalize and mean-center the image
model.add(Lambda(lambda x: x / 255.0 - 0.5))
# resize the image to the size used in the NVIDIA paper
...
# 2 convolutional layers with 3x3 kernel size
